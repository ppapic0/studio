...
function areStudentAttendanceRequestsEnabled(): boolean {
  return false;
}
...
type AttendanceAwayKind = "short" | "long";
...
type ApplyAttendanceStatusTransitionParams = {
  ...
  suppressParentSms?: boolean | null;
  nowMs?: number;
};
...
function normalizeAttendanceAwayKind(value: unknown): AttendanceAwayKind | null {
  const normalized = asTrimmedString(value).toLowerCase();
...
function getStudyBoxHourFromDailyPointEvent(event: DailyPointEventDoc): number | null {
  if (event.source !== "study_box") return null;

  const hour = Math.round(parseFiniteNumber(event.hour) ?? Number.NaN);
  if (Number.isFinite(hour) && hour >= 1 && hour <= 8) return hour;

  const labelMatch = event.label.trim().match(/^(\d+)\s*시간\s*상자$/);
  const labelHour = Math.round(parseFiniteNumber(labelMatch?.[1]) ?? Number.NaN);
  if (Number.isFinite(labelHour) && labelHour >= 1 && labelHour <= 8) return labelHour;

  return null;
}

function getStudyBoxOpenedHoursFromPointEvents(dayStatus: Record<string, unknown>): number[] {
  return normalizeStudyBoxHoursFromUnknown(
    normalizeDailyPointEvents(dayStatus.pointEvents)
      .map((entry) => getStudyBoxHourFromDailyPointEvent(entry))
      .filter((hour): hour is number => hour !== null)
  );
}

function getPointEventAwardTotal(dayStatus: Record<string, unknown>, source: DailyPointEventDoc["source"]): number {
...
function getCreditedStudyBoxPointTotal(dayStatus: Record<string, unknown>): number {
  ...
  return Math.max(
    0,
    getPointEventAwardTotal(dayStatus, "study_box"),
    dailyPointAmount - nonStudyBoxPointTotal
  );
}

function getStudyBoxPointEventCreditSummary(dayStatus: Record<string, unknown>) {
  const creditedByHour = new Map<number, number>();

  normalizeDailyPointEvents(dayStatus.pointEvents).forEach((event) => {
    if (event.source !== "study_box") return;

    const points = Math.max(0, Math.floor(parseFiniteNumber(event.points) ?? 0));
    if (points <= 0) return;

    const eventHour = getStudyBoxHourFromDailyPointEvent(event);
    if (eventHour === null) return;

    creditedByHour.set(eventHour, (creditedByHour.get(eventHour) ?? 0) + points);
  });

  return { creditedByHour };
}

function getStudyBoxRewardPointsForHour(
  rewardEntries: SecureStudyBoxReward[],
  hour: number,
  fallbackReward?: Pick<SecureStudyBoxReward, "awardedPoints" | "basePoints"> | null
) {
  const storedReward = rewardEntries.find((entry) => entry.milestone === hour) ?? null;
  const awardedPoints = Math.max(
    0,
    Math.floor(parseFiniteNumber(storedReward?.awardedPoints) ?? 0),
    Math.floor(parseFiniteNumber(fallbackReward?.awardedPoints) ?? 0),
    Math.floor(parseFiniteNumber(storedReward?.basePoints) ?? 0),
    Math.floor(parseFiniteNumber(fallbackReward?.basePoints) ?? 0)
  );
...
function getMissingStudyBoxCreditDelta(params: {
  ...
  if (!openedStudyBoxes.includes(hour)) return 0;
  ...
  for (const openedHour of openedStudyBoxes) {
    ...
    if (openedRewardPoints <= 0) continue;

    const specificCredits = Math.min(openedRewardPoints, Math.max(0, creditedByHour.get(openedHour) ?? 0));
...
function getPlanCompletionAwardTotal(dayStatus: Record<string, unknown>): number {
  const eventPoints = getPointEventAwardTotal(dayStatus, "plan_completion");
  const storedCount = Math.max(0, Math.floor(parseFiniteNumber(dayStatus.planCompletionRewardCount) ?? 0));
...
function buildStudyBoxPointEvent(
  existing: unknown,
  params: {
    dateKey: string;
    hour: number;
  ...
  const existingPoints = Math.max(
    0,
    Math.floor(
      parseFiniteNumber(normalizeDailyPointEvents(existing).find((entry) => entry.id === id)?.points) ?? 0
    )
  );
...
function resolveOpenedStudyBoxHoursFromDayStatus(dayStatus: Record<string, unknown>): number[] {
  ...
  const studyBoxAwardedPoints = Math.max(
    0,
    studyBoxEventPoints,
    persistedDailyPointAmount - getRankRewardAwardTotal(dayStatus) - getPlanCompletionAwardTotal(dayStatus)
  );
...
function getRecentDateKeys(dateKey: string, count: number): string[] {
  const keys: string[] = [];
  let cursor = dateKey;
...
function getPreviousStudyDayKey(baseDate: Date = new Date()): string {
  return getPreviousDateKey(toStudyDayKey(baseDate));
}

function isStudyBoxCarryoverOpenable(dateKey: string, baseDate: Date = new Date()): boolean {
  return dateKey === getPreviousStudyDayKey(baseDate) && !hasStudyBoxCarryoverExpired(dateKey, baseDate);
}

function getExpiredStudyBoxCarryoverDateKeys(
  dailyPointStatus: Record<string, unknown>,
...
function shouldSuppressAttendanceEventParentSms(eventData: Record<string, unknown>): boolean {
  const meta = asRecord(eventData.meta);
  return (
    eventData.suppressParentSms === true ||
    eventData.parentSmsSuppressed === true ||
    meta?.suppressParentSms === true ||
    meta?.parentSmsSuppressed === true
  );
}

function resolveAttendanceEventParentSmsSuppressedReason(eventData: Record<string, unknown>): string {
  const meta = asRecord(eventData.meta);
  return (
    asTrimmedString(eventData.parentSmsSuppressedReason) ||
    asTrimmedString(eventData.smsSuppressedReason) ||
    asTrimmedString(meta?.parentSmsSuppressedReason) ||
    asTrimmedString(meta?.smsSuppressedReason) ||
...
export const completeSignupWithInvite = functions.region(region).https.onCall(async (data, context) => {
  ...
  const uid = context.auth.uid;
  const roleInput = data?.role ?? data?.selectedRole ?? data?.intendedRole ?? data?.accountRole;
  ...
  if (!role) {
    ...
    const inviteRole = inviteSnapForRole.exists
      ? normalizeSignupRole((inviteSnapForRole.data() as Partial<InviteDoc> | undefined)?.intendedRole)
      : null;
    if (inviteRole) {
      role = inviteRole;
...
function buildAttendanceSeatPatch(params: {
  ...
  activeAwayKind?: AttendanceAwayKind | null;
  activeAwayParentSmsSuppressed?: boolean | null;
...
type AttendanceEventAwayContext = {
  awayKind: AttendanceAwayKind | null;
...
function buildAttendanceEventAwayContext(params: {
  ...
  if (params.eventType === "away_end") {
    ...
    const suppressParentSms =
      params.suppressParentSms === true ||
...
function buildAttendanceEventAwayPatch(context: AttendanceEventAwayContext): Record<string, unknown> {
  return {
    ...(context.awayKind ? { awayKind: context.awayKind } : {}),
    ...(context.suppressParentSms
      ? {
...
async function applyAttendanceStatusTransition(
  ...
  const sessionFinalizeSkipReason =
    isLeavingActiveStudy && preflightStartMs <= 0
  ...
  const shouldAllowAwayWithoutSession =
    preflightStatus === "studying" &&
    (nextStatus === "away" || nextStatus === "break") &&
    Boolean(sessionFinalizeSkipReason);
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (prevStatus === nextStatus) {
      if (params.source === "kiosk") {
        ...
        transaction.set(repeatEventRef, {
          ...
          repeatAction: true,
          ...buildAttendanceEventAwayPatch(repeatAwayContext),
    ...
    if (eventType && eventRef) {
      transaction.set(eventRef, {
        ...
        statusBefore: prevStatus,
        statusAfter: nextStatus,
        ...buildAttendanceEventAwayPatch(eventAwayContext),
        ...(sessionFinalizeSkipReason ? { sessionFinalizeSkipped: true, sessionFinalizeSkipReason } : {}),
        ...(params.actorUid ? { actorUid: params.actorUid } : {}),
      });
    }
...
export const setStudentAttendanceStatusSecure = attendanceMutationFunctions.https.onCall(async (data, context) => {
  ...
  let result: ApplyAttendanceStatusTransitionResult;
  try {
...
type KioskAttendanceQueueAction = "check_in" | "away_start" | "away_start_long" | "away_end" | "check_out";
...
function getKioskActionAwayKind(action: KioskAttendanceQueueAction): AttendanceAwayKind | null {
...
function isKioskActionAllowedFromStatus(
  ...
  if (action === "away_start" || action === "away_start_long") return status === "studying";
...
export const openStudyRewardBoxSecure = functions.region(region).https.onCall(async (data, context) => {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    const nextPointEvents = awardedDelta > 0
      ? upsertDailyPointEvent(
          currentDayStatus.pointEvents,
          buildStudyBoxPointEvent(currentDayStatus.pointEvents, {
            dateKey,
            hour,
            awardedDelta,
            nowMs,
    ...
    for (const plan of resolvedRewardPlans) {
      ...
      const creditRepairDayStatus = alreadyOpened
        ? {
            ...
            pointEvents: nextPointEvents,
            dailyPointAmount:
              Math.max(0, Math.floor(parseFiniteNumber(currentDayStatus.dailyPointAmount) ?? 0)) +
              awardedTotalDelta,
          }
        : currentDayStatus;
      const missingStudyBoxCreditDelta = alreadyOpened
      ...
      if (awardedDelta > 0) {
        nextPointEvents = upsertDailyPointEvent(
          nextPointEvents,
          buildStudyBoxPointEvent(nextPointEvents, {
            dateKey,
            hour: plan.hour,
            awardedDelta,
            nowMs,
  ...
  .onRun(async () => {
    ...
    for (const centerDoc of centersSnap.docs) {
      ...
      for (const progressDoc of progressSnap.docs) {
        ...
        if (expiredDateKeys.length === 0) continue;

        const dailyPointStatusUpdate: Record<string, unknown> = {};
        ...
        if (Object.keys(dailyPointStatusUpdate).length === 0) continue;
