...
const COUNSELING_ATTACHMENT_RETENTION_DAYS = 7;
const COUNSELING_ATTACHMENT_RETENTION_MS = COUNSELING_ATTACHMENT_RETENTION_DAYS * 24 * 60 * 60 * 1000;
const COUNSELING_QUESTION_ATTACHMENT_LIMIT = 3;
const COUNSELING_QUESTION_MAX_ATTACHMENT_BYTES = 8 * 1024 * 1024;
const COUNSELING_QUESTION_SUBJECTS = new Set(["국어", "수학", "영어", "탐구", "기타"]);
...
type CounselingQuestionAttachmentRequest = {
  id?: string | null;
  name?: string | null;
  path?: string | null;
...
type CounselingQuestionAttachmentDoc = {
  id: string;
  name: string;
  path: string;
  downloadUrl: string;
...
type CounselingAvailabilitySlotDoc = {
  centerId?: string;
  teacherId?: string;
  teacherName?: string;
  startsAt?: admin.firestore.Timestamp;
  endsAt?: admin.firestore.Timestamp;
  slotType?: "study_question" | string;
  note?: string | null;
  capacity?: number | null;
  reservedCount?: number | null;
  activeReservationIds?: string[] | null;
...
function normalizeCounselingQuestionSubject(value: unknown): string | null {
  const subject = asTrimmedString(value);
  if (!subject) return null;
  return COUNSELING_QUESTION_SUBJECTS.has(subject) ? subject : null;
}

function buildStudyQuestionReservationPreview(params: {
  subject?: string | null;
  ...
  const parts = [
    asTrimmedString(params.subject),
    [asTrimmedString(params.workbook), asTrimmedString(params.problemNumbers)].filter(Boolean).join(" "),
...
function getCounselingAttachmentCleanupTimestamp(baseDate = new Date()): admin.firestore.Timestamp {
  return admin.firestore.Timestamp.fromMillis(baseDate.getTime() + COUNSELING_ATTACHMENT_RETENTION_MS);
}
...
function normalizeCounselingQuestionAttachments(params: {
  attachments: unknown;
  centerId: string;
  studentId: string;
  uploadedAt: admin.firestore.Timestamp;
}): CounselingQuestionAttachmentDoc[] {
  const expectedPathPrefix = `centers/${params.centerId}/counseling-question-attachments/${params.studentId}/`;
  if (!Array.isArray(params.attachments) || params.attachments.length === 0) return [];
  if (params.attachments.length > COUNSELING_QUESTION_ATTACHMENT_LIMIT) {
    throw new functions.https.HttpsError("invalid-argument", "Too many attachments.", {
      userMessage: `사진은 최대 ${COUNSELING_QUESTION_ATTACHMENT_LIMIT}장까지 첨부할 수 있습니다.`,
    });
  }

  const uniquePaths = new Set<string>();
  return params.attachments.map((rawAttachment, index) => {
    const attachment = (rawAttachment || {}) as CounselingQuestionAttachmentRequest;
    const id = asTrimmedString(attachment.id, `attachment-${index + 1}`);
    const name = asTrimmedString(attachment.name, `question-${index + 1}.jpg`);
    ...
    if (!path || !path.startsWith(expectedPathPrefix)) {
      throw new functions.https.HttpsError("invalid-argument", "Attachment path is invalid.", {
        userMessage: "첨부 파일 경로가 올바르지 않습니다. 다시 업로드해 주세요.",
      });
    }
    if (!downloadToken || downloadToken.length < 8) {
      throw new functions.https.HttpsError("invalid-argument", "Attachment token is invalid.", {
        userMessage: "첨부 파일 정보를 다시 확인해 주세요.",
      });
    }
    if (!Number.isFinite(sizeBytes) || sizeBytes <= 0 || sizeBytes > COUNSELING_QUESTION_MAX_ATTACHMENT_BYTES) {
      throw new functions.https.HttpsError("invalid-argument", "Attachment size is invalid.", {
        userMessage: "사진 용량이 너무 크거나 올바르지 않습니다. 다시 업로드해 주세요.",
      });
    }
    if (uniquePaths.has(path)) {
      throw new functions.https.HttpsError("invalid-argument", "Duplicate attachment path.", {
        userMessage: "같은 사진이 중복 첨부되었습니다. 다시 확인해 주세요.",
      });
    }
...
export const requestCounselingReservationSecure = functions.region(region).https.onCall(async (data, context) => {
  const db = admin.firestore();
  ...
  const slotId = asTrimmedString(data?.slotId);
  const questionSubject = normalizeCounselingQuestionSubject(data?.questionSubject);
  const questionWorkbook = asTrimmedString(data?.questionWorkbook);
  const questionProblemNumbers = asTrimmedString(data?.questionProblemNumbers);
  const questionSummary = asTrimmedString(data?.questionSummary);
  const questionDetails = asTrimmedString(data?.questionDetails);

  if (!centerId || !slotId) {
    throw new functions.https.HttpsError("invalid-argument", "centerId and slotId are required.", {
      userMessage: "예약할 상담 슬롯을 다시 선택해 주세요.",
    });
  }
  if (!questionSubject) {
    throw new functions.https.HttpsError("invalid-argument", "questionSubject is required.", {
      userMessage: "과목을 선택해 주세요.",
    });
  }
  if (questionWorkbook.length < 2) {
    throw new functions.https.HttpsError("invalid-argument", "questionWorkbook is too short.", {
      userMessage: "문제집 또는 자료명을 입력해 주세요.",
    });
  }
  if (questionProblemNumbers.length < 1) {
    throw new functions.https.HttpsError("invalid-argument", "questionProblemNumbers is required.", {
      userMessage: "질문할 문제 번호나 범위를 입력해 주세요.",
    });
  }
  if (questionSummary.length < 4) {
    throw new functions.https.HttpsError("invalid-argument", "questionSummary is too short.", {
      userMessage: "질문 요약은 4자 이상 입력해 주세요.",
    });
  }
  if (questionDetails.length > 2000) {
    throw new functions.https.HttpsError("invalid-argument", "questionDetails is too long.", {
      userMessage: "상세 질문은 2000자 이하로 입력해 주세요.",
    });
  }
  ...
  if (membership.role !== "student" || !isActiveMembershipStatus(membership.status)) {
    throw new functions.https.HttpsError("permission-denied", "Only active students can create study-question reservations.", {
      userMessage: "학생 본인만 학습 질의 상담을 신청할 수 있습니다.",
    });
  }
  ...
  if (!studentIdentity) {
    throw new functions.https.HttpsError("failed-precondition", "Student profile not found.", {
      userMessage: "학생 정보를 찾지 못했습니다. 다시 로그인 후 시도해 주세요.",
    });
  }
  ...
  const questionAttachments = normalizeCounselingQuestionAttachments({
    attachments: data?.questionAttachments,
    centerId,
    studentId,
    uploadedAt,
  });
  const preview = buildStudyQuestionReservationPreview({
    subject: questionSubject,
    workbook: questionWorkbook,
    problemNumbers: questionProblemNumbers,
    summary: questionSummary,
  });

  const slotRef = db.doc(`centers/${centerId}/counselingAvailabilitySlots/${slotId}`);
  const reservationRef = db.collection(`centers/${centerId}/counselingReservations`).doc();
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!slotSnap.exists) {
      throw new functions.https.HttpsError("not-found", "Counseling slot not found.", {
        userMessage: "선택한 질의 상담 시간이 더 이상 없습니다. 다른 시간을 선택해 주세요.",
      });
    }

    const slotData = slotSnap.data() as CounselingAvailabilitySlotDoc;
    const startsAt = toTimestampOrNow(slotData.startsAt);
    const endsAt = toTimestampOrNow(slotData.endsAt);
    teacherId = asTrimmedString(slotData.teacherId);
    teacherName = asTrimmedString(slotData.teacherName, "선생님");

    if (asTrimmedString(slotData.centerId, centerId) !== centerId) {
      throw new functions.https.HttpsError("failed-precondition", "Slot center mismatch.", {
        userMessage: "선택한 질의 상담 슬롯이 올바르지 않습니다.",
      });
    }
    if (slotData.slotType && slotData.slotType !== "study_question") {
      throw new functions.https.HttpsError("failed-precondition", "Slot type is invalid.", {
        userMessage: "학습 질의 상담 전용 시간만 예약할 수 있습니다.",
      });
    }
    if (slotData.isPublished === false) {
      throw new functions.https.HttpsError("failed-precondition", "Slot is not published.", {
        userMessage: "아직 공개되지 않은 시간입니다. 다른 슬롯을 선택해 주세요.",
      });
    }
    if (!teacherId || !teacherName || !startsAt || !endsAt || endsAt.toMillis() <= startsAt.toMillis()) {
      throw new functions.https.HttpsError("failed-precondition", "Slot data is invalid.", {
        userMessage: "질의 상담 슬롯 정보가 올바르지 않습니다. 센터에 문의해 주세요.",
      });
    }
    if (startsAt.toMillis() <= Date.now()) {
      throw new functions.https.HttpsError("failed-precondition", "Slot has already started.", {
        userMessage: "이미 시작된 시간은 예약할 수 없습니다. 다른 시간을 선택해 주세요.",
      });
    }

    const capacity = Math.max(1, Math.round(parseFiniteNumber(slotData.capacity) ?? 1));
    const activeReservationIds = normalizeStringArray(slotData.activeReservationIds);
    const reservedCount = Math.max(
      0,
      Math.round(parseFiniteNumber(slotData.reservedCount) ?? activeReservationIds.length)
    );
    const effectiveReservedCount = Math.max(reservedCount, activeReservationIds.length);

    if (effectiveReservedCount >= capacity) {
      throw new functions.https.HttpsError("resource-exhausted", "Slot capacity exceeded.", {
        userMessage: "해당 시간은 예약이 마감되었습니다. 다른 시간을 선택해 주세요.",
      });
    }

    for (const activeReservationId of activeReservationIds.slice(0, Math.max(capacity, 8))) {
      const activeReservationRef = db.doc(`centers/${centerId}/counselingReservations/${activeReservationId}`);
      const activeReservationSnap = await transaction.get(activeReservationRef);
      if (!activeReservationSnap.exists) continue;

      const activeReservation = activeReservationSnap.data() as { studentId?: unknown; status?: unknown };
      const activeStudentId = asTrimmedString(activeReservation.studentId);
      const activeStatus = asTrimmedString(activeReservation.status, "requested");
      if (activeStudentId === studentId && activeStatus !== "canceled") {
        throw new functions.https.HttpsError("already-exists", "Student already booked this slot.", {
          userMessage: "이미 같은 시간의 질의 상담을 신청했습니다.",
        });
      }
    }
    ...
    transaction.set(
      reservationRef,
      {
        ...
        questionSubject,
        questionWorkbook,
        questionProblemNumbers,
        questionSummary,
        questionDetails: questionDetails || null,
        questionAttachments,
        questionAttachmentCleanupAt: null,
        questionAttachmentsDeletedAt: null,
        doneAt: null,
        canceledAt: null,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    ...
    transaction.set(
      slotRef,
      {
        reservedCount: effectiveReservedCount + 1,
        activeReservationIds: Array.from(new Set([...activeReservationIds, reservationRef.id])),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    ...
    scheduledAtMs = startsAt.toMillis();
  });
...
export const cancelCounselingReservationSecure = functions.region(region).https.onCall(async (data, context) => {
  const db = admin.firestore();
  ...
  const centerId = asTrimmedString(data?.centerId);
  const reservationId = asTrimmedString(data?.reservationId);

  if (!centerId || !reservationId) {
    throw new functions.https.HttpsError("invalid-argument", "centerId and reservationId are required.", {
      userMessage: "취소할 상담 예약을 다시 선택해 주세요.",
    });
  }

  const membership = await resolveCenterMembershipRole(db, centerId, authUid);
  if (!membership.role || !isActiveMembershipStatus(membership.status)) {
    throw new functions.https.HttpsError("permission-denied", "Inactive membership cannot cancel counseling reservations.", {
      userMessage: "현재 계정 상태로는 상담을 취소할 수 없습니다.",
    });
  }

  const callerStudentIdentity = membership.role === "student"
    ? await resolveCenterStudentIdentity(db, centerId, authUid)
    : null;

  const reservationRef = db.doc(`centers/${centerId}/counselingReservations/${reservationId}`);
  let alreadyCanceled = false;

  await db.runTransaction(async (transaction) => {
    const reservationSnap = await transaction.get(reservationRef);
    if (!reservationSnap.exists) {
      throw new functions.https.HttpsError("not-found", "Counseling reservation not found.", {
        userMessage: "상담 예약을 찾지 못했습니다.",
      });
    }
    ...
    const reservation = reservationSnap.data() as {
      ...
      questionAttachments?: unknown;
      questionAttachmentsDeletedAt?: unknown;
    };
    const reservationStudentId = asTrimmedString(reservation.studentId);
    const reservationTeacherId = asTrimmedString(reservation.teacherId);
    const status = asTrimmedString(reservation.status, "requested");
    const availabilitySlotId = asTrimmedString(reservation.availabilitySlotId);
    const callerStudentId = callerStudentIdentity?.studentId || authUid;
    const canCancelAsStudent = membership.role === "student"
      && (reservationStudentId === callerStudentId || reservationStudentId === authUid);
    const canCancelAsStaff = isTeacherOrAdminMembership(membership.role)
      || (membership.role === "teacher" && reservationTeacherId === authUid);

    if (!canCancelAsStudent && !canCancelAsStaff) {
      throw new functions.https.HttpsError("permission-denied", "Caller cannot cancel this reservation.", {
        userMessage: "이 상담 예약을 취소할 권한이 없습니다.",
      });
    }
    if (status === "done") {
      throw new functions.https.HttpsError("failed-precondition", "Completed counseling cannot be cancelled.", {
        userMessage: "이미 완료된 상담은 취소할 수 없습니다.",
      });
    }

    alreadyCanceled = status === "canceled";
    const attachments = Array.isArray(reservation.questionAttachments)
      ? (reservation.questionAttachments as Array<{ path?: unknown }>)
      : [];
    const hasAttachments = attachments.some((attachment) => asTrimmedString(attachment?.path).length > 0);
    const attachmentsDeletedAt = toTimestampOrNow(reservation.questionAttachmentsDeletedAt);

    transaction.set(
      reservationRef,
      {
        status: "canceled",
        canceledAt: alreadyCanceled ? (toTimestampOrNow((reservation as any).canceledAt) || admin.firestore.FieldValue.serverTimestamp()) : admin.firestore.FieldValue.serverTimestamp(),
        questionAttachmentCleanupAt: hasAttachments && !attachmentsDeletedAt
          ? getCounselingAttachmentCleanupTimestamp()
          : null,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true }
    ...
    if (availabilitySlotId && !alreadyCanceled) {
      const slotRef = db.doc(`centers/${centerId}/counselingAvailabilitySlots/${availabilitySlotId}`);
      const slotSnap = await transaction.get(slotRef);
      if (slotSnap.exists) {
        const slotData = slotSnap.data() as CounselingAvailabilitySlotDoc;
        const activeReservationIds = normalizeStringArray(slotData.activeReservationIds);
        const nextActiveReservationIds = activeReservationIds.filter((id) => id !== reservationId);
        const currentReservedCount = Math.max(
          0,
          Math.round(parseFiniteNumber(slotData.reservedCount) ?? activeReservationIds.length)
        );
        const nextReservedCount = Math.max(nextActiveReservationIds.length, currentReservedCount - 1);

        transaction.set(
          slotRef,
          {
            reservedCount: nextReservedCount,
            activeReservationIds: nextActiveReservationIds,
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            updatedByUid: authUid,
          },
          { merge: true }
...
export const scheduledCounselingQuestionAttachmentCleanup = functions
  .region(region)
  .runWith({
    timeoutSeconds: 540,
    memory: "1GB",
  ...
  .onRun(async () => {
    ...
    for (const centerDoc of centersSnap.docs) {
      ...
      const reservationsSnap = await db
        ...
        .where("questionAttachmentCleanupAt", "<=", now)
        .limit(250)
        .get();
      ...
      for (const reservationDoc of reservationsSnap.docs) {
        ...
        if (toTimestampOrNow(reservation.questionAttachmentsDeletedAt)) {
          skippedReservations += 1;
          continue;
        }

        const rawAttachments = Array.isArray(reservation.questionAttachments)
          ? (reservation.questionAttachments as Array<Record<string, unknown>>)
          : [];
        const attachmentPaths = Array.from(
          new Set(
            rawAttachments
              .map((attachment) => asTrimmedString(attachment?.path))
        ...
        for (const path of attachmentPaths) {
          try {
            ...
            deletionFailed = true;
            console.error("[counseling] attachment delete failed", {
              centerId,
        ...
        const nextAttachments = rawAttachments.map((attachment, index) => {
          const sizeBytes = Math.round(parseFiniteNumber(attachment?.sizeBytes) ?? 0);
          const width = Math.round(parseFiniteNumber(attachment?.width) ?? Number.NaN);
          const height = Math.round(parseFiniteNumber(attachment?.height) ?? Number.NaN);

          return {
            id: asTrimmedString(attachment?.id, `attachment-${index + 1}`),
            name: asTrimmedString(attachment?.name, `question-${index + 1}.jpg`),
            path: asTrimmedString(attachment?.path),
            downloadUrl: "",
            contentType: asTrimmedString(attachment?.contentType, "image/jpeg"),
            sizeBytes: Math.max(0, sizeBytes),
            width: Number.isFinite(width) && width > 0 ? width : null,
            height: Number.isFinite(height) && height > 0 ? height : null,
            uploadedAt: toTimestampOrNow(attachment?.uploadedAt) || null,
            deletedAt: now,
          };
        });

        await reservationDoc.ref.set({
          questionAttachments: nextAttachments,
          questionAttachmentsDeletedAt: now,
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });

        cleanedReservations += 1;
      }
    }

    functions.logger.info("counseling question attachment cleanup complete", {
      centerCount: centersSnap.size,
      scannedReservations,
      cleanedReservations,
      deletedFiles,
