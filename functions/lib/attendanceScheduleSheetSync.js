...
function addFallbackStudentKey(fallbackStudentIdsByKey, params) {
    if (!normalizeLookupText(params.name))
        return;
    ...
    if (existingStudentIds.includes(params.studentId))
        return;
...
async function loadStudentsForCenter(db, centerId) {
    ...
    memberDocs.forEach((memberDoc, index) => {
        ...
        if (schoolName)
            addFallbackStudentKey(fallbackStudentIdsByKey, { studentId, name: studentName, school: schoolName });
        if (grade)
            addFallbackStudentKey(fallbackStudentIdsByKey, { studentId, name: studentName, grade });
...
function matchStudentFromSheetRow(params) {
    if (params.identifier) {
        ...
        if (directMatch)
            return { matchedStudent: directMatch, duplicate: false };
    }
