  ...
  return (
    <div className={cn("flex flex-col gap-8 max-w-6xl mx-auto pb-20", isMobile ? "gap-4 px-1" : "gap-10 px-4 py-6")}>
      <header className="flex flex-col gap-1 px-1">
        <h1 className={cn("font-black tracking-tighter text-primary", isMobile ? "text-2xl" : "text-4xl")}>실시간 관제 홈</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="rounded-[2rem] border-none shadow-lg bg-blue-50/50 group">
          <CardContent className="p-6 flex flex-col gap-1">
            <div className="flex items-center justify-between text-blue-600 mb-2"><Activity className="h-5 w-5" /><span className="text-[10px] font-black uppercase tracking-widest">학습 중</span></div>
        ...
        <Card className="rounded-[2rem] border-none shadow-lg bg-rose-50/50">
          <CardContent className="p-6 flex flex-col gap-1">
            <div className="flex items-center justify-between text-rose-600 mb-2"><AlertCircle className="h-5 w-5" /><span className="text-[10px] font-black uppercase tracking-widest">미입실</span></div>
        ...
        <Card className="rounded-[2rem] border-none shadow-lg bg-white hidden md:flex"><CardContent className="p-6 flex flex-col gap-1 w-full"><div className="flex items-center justify-between text-muted-foreground mb-2"><Armchair className="h-5 w-5" /><span className="text-[10px] font-black uppercase tracking-widest">총 좌석</span></div><div className="text-4xl font-black text-primary tracking-tighter">{attendanceList?.length || 0}<span className="text-lg ml-1 opacity-40">석</span></div></CardContent></Card>
        <Card className="rounded-[2rem] border-none shadow-lg bg-white hidden md:flex"><CardContent className="p-6 flex flex-col gap-1 w-full"><div className="flex items-center justify-between text-muted-foreground mb-2"><Users className="h-5 w-5" /><span className="text-[10px] font-black uppercase tracking-widest">등록 학생</span></div><div className="text-4xl font-black text-primary tracking-tighter">{students?.length || 0}<span className="text-lg ml-1 opacity-40">명</span></div></CardContent></Card>
      </div>
      ...
      <div className="grid gap-8">
          ...
          <CardHeader className="bg-muted/5 border-b p-8 sm:p-10">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-2xl font-black flex items-center gap-3 tracking-tighter"><Armchair className="h-6 w-6 text-primary" /> 실시간 좌석 상황판</CardTitle>
              ...
              <Button asChild variant="outline" size="sm" className="rounded-2xl h-11 px-6 font-black border-2 shadow-sm bg-white"><Link href="/dashboard/teacher/layout-view">전체 도면 보기</Link></Button>
            </div>
          </CardHeader>
          <CardContent className={cn("p-6 sm:p-10 bg-[#fafafa]")}>
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-40 gap-4"><Loader2 className="animate-spin h-10 w-10 text-primary opacity-20" /><p className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/40 italic">Syncing live data...</p></div>
                ...
                {students.map((student) => {
                  ...
                  return (
                    <Link key={student.id} href={`/dashboard/teacher/students/${student.id}`}>
                      <div className={cn("aspect-square rounded-2xl border-2 flex flex-col items-center justify-center gap-1 transition-all active:scale-90 relative overflow-hidden group shadow-sm p-2",
                        isStudying ? "bg-blue-600 border-blue-700 text-white shadow-xl scale-105 z-10" : isAway ? "bg-amber-500 border-amber-600 text-white shadow-lg" : "bg-white border-muted-foreground/5 text-muted-foreground hover:border-primary/20")}>
                        ...
                        <span className={cn("text-[9px] font-bold tracking-tighter opacity-80", isStudying || isAway ? "text-white" : "text-primary/60")}>{getLiveTimeLabel(seatId, student.id)}</span>
                        {isStudying && <div className="absolute bottom-1.5"><Activity className="h-2 w-2 animate-pulse" /></div>}
                      </div>
                    </Link>
                  );
                })}
        ...
        <Card className="rounded-[3rem] border-none shadow-2xl bg-white overflow-hidden ring-1 ring-border/50">
          <CardHeader className="p-8 sm:p-10 border-b bg-muted/5">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-2xl font-black flex items-center gap-3 tracking-tighter"><MessageSquare className="h-6 w-6 text-primary" /> 오늘 상담 일정</CardTitle>
          ...
          <CardContent className="p-0">
            {aptLoading ? (
              ...
              <div className="py-24 text-center flex flex-col items-center gap-4 text-muted-foreground/30 font-black italic"><MessageSquare className="h-12 w-12 opacity-10" /><p className="text-sm">오늘 예정된 상담이 없습니다.</p></div>
            ) : (
                  ...
                  <Link key={apt.id} href={`/dashboard/teacher/students/${apt.studentId}`}>
                    <div className="p-8 flex items-center justify-between hover:bg-muted/5 transition-all group">
                      <div className="flex items-center gap-8">
                        ...
                        <div className="space-y-1"><h3 className="text-xl font-black group-hover:text-primary transition-colors">{apt.studentName} 학생</h3><p className="text-sm font-bold text-muted-foreground">{apt.studentNote || '상담 주제 미입력'}</p></div>
                      </div>
                      <div className="flex items-center gap-4">{getStatusBadge(apt.status)}<ChevronRight className="h-6 w-6 text-muted-foreground/20 group-hover:translate-x-1 group-hover:text-primary transition-all" /></div>
                    </div>
                  </Link>
                ))}
              </div>
