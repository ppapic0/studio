    ...
    <div className={cn("flex flex-col w-full max-w-5xl mx-auto pb-20", isMobile ? "gap-4 px-1" : "gap-10")}>
      ...
      <Card className="relative mx-auto w-full overflow-hidden rounded-[3rem] border border-emerald-100/80 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.08),transparent_24%),linear-gradient(180deg,#ffffff_0%,#f8fcff_100%)] shadow-[0_28px_70px_-52px_rgba(15,23,42,0.4)] ring-1 ring-white/70">
        <CardContent className="relative p-0">
          ...
          <div className={cn("grid grid-cols-7 border-b border-primary/10", isMobile ? "gap-[3px] px-1.5 py-2" : "gap-1.5 px-4 py-3")}>
            {['월', '화', '수', '목', '금', '토', '일'].map((day, i) => (
              <div
                key={day}
                className={cn(
                    isMobile ? "py-1.5 text-[9px]" : "py-3 text-[11px]",
          ...
          <div className={cn("grid grid-cols-7", isMobile ? "auto-rows-fr gap-[3px] p-1.5" : "auto-rows-fr gap-3 p-4")}>
            {logsLoading ? (
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isMobile ? "aspect-square min-h-[3.95rem] p-1" : "min-h-[150px] p-3",
                    ...
                    isTodayCalendar && (isMobile ? "z-10 ring-2 ring-inset ring-primary/30 shadow-[0_12px_24px_-18px_rgba(37,99,235,0.2)]" : "z-10 -translate-y-[1px] ring-2 ring-inset ring-primary/35 shadow-[0_20px_40px_-22px_rgba(37,99,235,0.22)]")
                  )}
                >
                  {isTodayCalendar && <div className={cn("pointer-events-none absolute border border-primary/20", isMobile ? "-inset-[1px] rounded-[1.1rem]" : "-inset-0.5 rounded-[1.35rem]")} />}
                  ...
                  <div className={cn("relative z-10 flex justify-between items-start gap-1", isMobile ? "mb-0.5" : "mb-2.5")}>
                    ...
                    {hasStatusCluster ? (
                      <div className={cn("inline-flex items-center gap-1 rounded-full border border-slate-200/85 bg-white/96 shadow-[0_10px_20px_-18px_rgba(15,23,42,0.24)]", isMobile ? "px-1 py-[0.18rem]" : "px-2 py-1")}>
                  ...
                  <div className={cn(isMobile ? "absolute inset-x-1 bottom-1 flex justify-center" : "absolute left-0 right-0 bottom-3 px-3")}>
                    <div
                      className={cn(
                        "border bg-white text-center whitespace-nowrap",
                        isMobile ? "inline-flex max-w-full items-center justify-center rounded-[0.8rem] px-1.5 py-[0.42rem] shadow-[0_10px_18px_-14px_rgba(15,23,42,0.2)]" : "rounded-[0.95rem] px-3 py-2.5 shadow-[0_16px_26px_-22px_rgba(15,23,42,0.26)]",
                        getCalendarTimeCapsuleClass(minutes, isCurrentMonth)
                      )}
                    >
                      {isMobile ? (
                        <span className="dashboard-number block tabular-nums text-[0.76rem] leading-none tracking-[-0.08em] min-[360px]:text-[0.82rem]">
                          {timeLabel}
                        </span>
                      ) : (
