...
const TIERS = [
  { name: '아이언', min: 0, color: 'text-slate-400', bg: 'bg-slate-400', border: 'border-slate-200', gradient: 'from-slate-500 to-slate-700', shadow: 'shadow-slate-200' },
  { name: '브론즈', min: 20, color: 'text-orange-700', bg: 'bg-orange-700', border: 'border-orange-200', gradient: 'from-orange-600 to-orange-800', shadow: 'shadow-orange-200' },
  { name: '실버', min: 40, color: 'text-slate-300', bg: 'bg-slate-300', border: 'border-slate-100', gradient: 'from-slate-300 to-slate-500', shadow: 'shadow-slate-100' },
  { name: '골드', min: 60, color: 'text-yellow-500', bg: 'bg-yellow-500', border: 'border-yellow-200', gradient: 'from-amber-400 to-yellow-600', shadow: 'shadow-yellow-200' },
  { name: '플래티넘', min: 75, color: 'text-emerald-400', bg: 'bg-emerald-400', border: 'border-emerald-200', gradient: 'from-emerald-400 to-teal-600', shadow: 'shadow-emerald-200' },
  { name: '다이아몬드', min: 85, color: 'text-blue-400', bg: 'bg-blue-400', border: 'border-blue-200', gradient: 'from-blue-400 to-indigo-600', shadow: 'shadow-blue-200' },
  { name: '마스터', min: 95, color: 'text-purple-500', bg: 'bg-purple-500', border: 'border-purple-200', gradient: 'from-purple-500 to-violet-700', shadow: 'shadow-purple-200' },
];
  ...
  const handleStudyStartStop = async () => {
    ...
    if (isTimerActive) {
      ...
      if (sessionMinutes > 0) {
        ...
        const seatRef = doc(firestore, 'centers', activeMembership.id, 'attendanceCurrent', `seat_${progress?.mastery?.toString().padStart(3, '0') || '001'}`);
      ...
      <section className={cn(
        ...
        <div className={cn("relative z-10 flex flex-col gap-8", isMobile ? "items-center text-center" : "md:flex-row md:justify-between md:text-left")}>
          ...
          <div className={cn("flex flex-col sm:flex-row items-center gap-5 sm:gap-8 w-full md:w-auto")}>
            ...
            >
              {isTimerActive ? <>종료 <Square className="ml-3 h-8 w-8 fill-current" /></> : <>트랙 시작 <Play className="ml-3 h-8 w-8 fill-current" /></>}
            </Button>
          </div>
        </div>
      </section>
      ...
      <div className={cn("grid gap-6 grid-cols-1 lg:grid-cols-3")}>
        <Card className={cn("border-none shadow-2xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/[0.03] lg:col-span-2")}>
          <CardHeader className="bg-muted/10 border-b p-10">
            <CardTitle className="font-black flex items-center gap-4 tracking-tighter text-3xl">
        ...
        <Card className="border-none shadow-2xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/[0.03]">
          <CardHeader className="bg-accent/5 border-b p-10">
            <CardTitle className="font-black flex items-center gap-4 tracking-tighter text-accent">
          ...
          <CardContent className="p-10 bg-[#fafafa]">
            <div className="space-y-4">
              {scheduleItems.length === 0 ? (
                ...
                <div key={item.id} className="flex items-center justify-between p-8 rounded-[2.5rem] bg-white border shadow-sm group hover:border-accent/30 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-2xl bg-accent/5 group-hover:bg-accent group-hover:text-white transition-all"><Timer className="h-4 w-4" /></div>
                    <span className="font-black tracking-tight">{item.title.split(': ')[0]}</span>
                  </div>
                  <Badge variant="outline" className="font-mono font-black text-accent text-lg px-4 py-1.5 rounded-2xl border-accent/20 bg-accent/5">
                    {item.title.split(': ')[1] || '--:--'}
                  </Badge>
                </div>
              ))}
