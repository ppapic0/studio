    ...
    <section id="class-system" className="scroll-mt-28 bg-[#F3F7FF] py-16 sm:py-20">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          ...
          description="입시학원 프로그램은 별도 등록 학생 대상이며, 그룹 수업 방식으로 수능 대비 중심 커리큘럼을 운영합니다. 수능특강 독서, 평가원, 사설 모의고사 흐름까지 실전 중심으로 다룹니다."
        />

        <div className="marketing-card mt-8 px-4 py-3 text-sm font-bold text-[#14295F] sm:px-5">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          {classSystem.map((step, index) => (
            <article key={step.title} className="marketing-card relative p-5">
              ...
              <h3 className="font-brand break-keep text-lg text-[#14295F]">{step.title}</h3>
