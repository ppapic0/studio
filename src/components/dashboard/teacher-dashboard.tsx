...
export function TeacherDashboard({ isActive }: { isActive: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1600px] mx-auto pb-24 min-h-screen">
      <header className={cn("flex justify-between px-4 pt-6 gap-4", isMobile ? "flex-col" : "flex-row items-center")}>
        <div className="flex items-center gap-3">
          ...
          <div className="grid">
            ...
            <p className={cn("font-bold text-muted-foreground uppercase tracking-[0.3em] mt-0.5 ml-1", isMobile ? "text-[8px]" : "text-[10px]")}>Command & Control Matrix</p>
          </div>
      ...
      <Card className={cn(
        ...
        <CardContent className={cn("flex justify-center", isMobile ? "p-4" : "p-6 sm:p-10")}>
          <ScrollArea className="w-full max-w-full">
            <div className={cn("rounded-[2rem] border-2 border-muted/30 bg-[#fafafa] w-fit mx-auto", isMobile ? "p-4" : "p-6 sm:p-8")}>
              <div 
      ...
      <section className={cn("space-y-6", isMobile ? "px-4" : "px-6")}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <MessageSquare className={cn("text-primary", isMobile ? "h-5 w-5" : "h-6 w-6")} />
            <h2 className={cn("font-black tracking-tighter", isMobile ? "text-xl" : "text-2xl")}>오늘 상담 현황</h2>
            <Badge variant="secondary" className="bg-primary/5 text-primary border-none font-black h-6">{appointments.length}건</Badge>
          </div>
          <Button asChild variant="ghost" className="font-black text-[10px] sm:text-xs text-muted-foreground hover:text-primary gap-2">
            <Link href="/dashboard/appointments">관리 <ArrowRight className="h-4 w-4" /></Link>
          </Button>
        </div>
        ...
        <div className={cn("grid gap-4", isMobile ? "grid-cols-1" : "md:grid-cols-2 lg:grid-cols-3")}>
          {aptLoading ? <div className="col-span-full py-10 flex justify-center"><Loader2 className="animate-spin h-8 w-8 text-primary opacity-20" /></div> : appointments.length === 0 ? (
            <div className="col-span-full py-16 text-center bg-white/50 rounded-[2.5rem] border-2 border-dashed border-muted-foreground/10">
              <p className="font-black text-muted-foreground/30 text-sm italic">예정된 상담이 없습니다.</p>
            </div>
          ) : appointments.map((apt) => (
            <Card key={apt.id} className="rounded-[2rem] border-none shadow-sm bg-white p-5 sm:p-6 flex items-center justify-between group hover:shadow-md transition-all active:scale-[0.98]">
              <div className="flex items-center gap-4">
                ...
                <div className="grid leading-tight min-w-0">
                  ...
                  <span className="text-[10px] font-bold text-muted-foreground truncate max-w-[120px]">{apt.studentNote || '상담 주제 미입력'}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge className={cn("font-black text-[8px] sm:text-[9px] border-none", apt.status === 'requested' ? "bg-amber-50 text-amber-600" : "bg-emerald-500 text-white")}>
      ...
      <Dialog open={isManaging} onOpenChange={setIsManaging}>
        <DialogContent className={cn("rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl flex flex-col", isMobile ? "fixed inset-0 w-full h-full max-w-none rounded-none" : "sm:max-w-lg")}>
          {selectedSeat && (
            <>
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#fafafa]">
                      <div className={cn("space-y-8", isMobile ? "p-6" : "p-8")}>
                        ...
                        {isEditMode ? (
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <Button onClick={() => handleStatusUpdate('studying')} className="h-20 sm:h-24 rounded-[2rem] font-black bg-blue-600 hover:bg-blue-700 text-white gap-2 flex flex-col items-center justify-center shadow-xl active:scale-95 transition-all">
                              <Zap className="h-5 w-5 fill-current" />
                              <span className="text-base sm:text-lg leading-none">입실 확인</span>
                            </Button>
                            <Button variant="outline" onClick={() => handleStatusUpdate('absent')} className="h-20 sm:h-24 rounded-[2rem] font-black border-2 border-rose-100 text-rose-600 hover:bg-rose-50 gap-2 flex flex-col items-center justify-center active:scale-95 transition-all">
                              <AlertCircle className="h-5 w-5" />
                              <span className="text-base sm:text-lg leading-none">퇴실 처리</span>
                            </Button>
                          </div>
                        )}
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between px-1">
                            <h4 className="text-[10px] font-black uppercase text-primary/60 tracking-widest flex items-center gap-2">
                              <History className="h-3 w-3" /> 오늘의 몰입 히스토리
                            </h4>
                          ...
                          <div className="grid gap-2">
                            {sessionsLoading ? (
                              ...
                              <div className="py-10 text-center rounded-2xl border-2 border-dashed border-muted-foreground/10 italic text-[10px] text-muted-foreground">
                                기록된 세션이 없습니다.
                              </div>
                            ) : (
                                ...
                                <div key={session.id} className="p-4 rounded-2xl bg-white border border-border/50 shadow-sm flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="grid leading-tight">
                                      ...
                                      <span className="text-[8px] font-bold text-muted-foreground uppercase">Captured</span>
                                    </div>
                                  </div>
                        ...
                        <div className="pt-4 border-t border-dashed">
                          <Button variant="secondary" className="w-full h-14 sm:h-16 rounded-2xl font-black gap-4 text-primary bg-primary/5 hover:bg-primary/10 transition-all border border-primary/5" asChild>
                            <Link href={`/dashboard/teacher/students/${selectedSeat.studentId}`}><User className="h-5 w-5 opacity-40" />분석 리포트 보기<ChevronRight className="ml-auto h-5 w-5 opacity-20" /></Link>
                          </Button>
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={isAssigning} onOpenChange={setIsAssigning}>
        <DialogContent className={cn("rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl flex flex-col", isMobile ? "fixed inset-0 w-full h-full max-w-none rounded-none" : "sm:max-w-md")}>
          <div className="bg-primary p-8 text-white relative shrink-0">
            ...
            <DialogHeader className="relative z-10">
              <DialogTitle className="text-2xl sm:text-3xl font-black tracking-tighter flex items-center gap-3">
                {selectedSeat?.type === 'aisle' ? <MapIcon className="h-7 w-7" /> : <UserPlus className="h-7 w-7" />}
                배정 설정
              </DialogTitle>
              <p className="text-white/60 font-bold mt-1 text-xs">GRID ID: {selectedSeat?.seatNo}</p>
            </DialogHeader>
          </div>
            ...
            {isEditMode && (
              <div className="p-4 rounded-2xl bg-muted/30 border-2 border-dashed border-primary/20">
                <Button 
                  ...
                  className={cn(
                    "w-full h-12 rounded-xl font-black gap-2 transition-all",
            ...
            {selectedSeat?.type !== 'aisle' && (
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input placeholder="이름 검색..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="rounded-xl border-2 pl-10 h-11 text-sm font-bold" />
                </div>
                    ...
                    {unassignedStudents.length === 0 ? <p className="text-center py-10 text-[10px] font-bold text-muted-foreground/40 italic">미배정 학생이 없습니다.</p> : unassignedStudents.map((student) => (
                      <div key={student.id} onClick={() => assignStudentToSeat(student)} className="p-4 rounded-2xl border-2 border-transparent hover:border-primary/10 hover:bg-primary/5 cursor-pointer flex items-center justify-between transition-all">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="h-9 w-9 rounded-xl bg-primary/5 flex items-center justify-center font-black text-primary border border-primary/10 shrink-0">{student.name.charAt(0)}</div>
                          <div className="grid gap-0.5 min-w-0"><span className="font-black text-sm truncate">{student.name}</span><span className="text-[10px] font-bold text-muted-foreground truncate">{student.schoolName}</span></div>
                        </div>
          ...
          <DialogFooter className={cn("p-6 bg-muted/20 border-t shrink-0 flex justify-center", isMobile ? "p-4" : "p-6")}>
            <Button variant="ghost" onClick={() => setIsAssigning(false)} className="w-full font-bold text-muted-foreground">취소</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
