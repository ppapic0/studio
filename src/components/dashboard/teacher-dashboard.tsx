...
export function TeacherDashboard({ isActive }: { isActive: boolean }) {
  ...
  // 3. 오늘 상담 예약 (counselingReservations 경로 사용)
    ...
    <div className="flex flex-col gap-6 sm:gap-8 w-full">
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: '평균 완수율', val: '88%', color: 'text-amber-500', icon: TrendingUp, sub: '센터 전체' },
          ...
          <CardHeader className="bg-muted/20 border-b p-5 sm:p-8">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <CardDescription className="font-bold text-xs sm:text-sm text-muted-foreground">도면을 클릭하여 학생 상세 정보를 확인하세요.</CardDescription>
              </div>
              <Button 
                variant="outline" 
              ...
              >
                <Settings2 className="h-4 w-4" /> 도면 수정
              </Button>
            </div>
          </CardHeader>
          <CardContent className="p-4 sm:p-8">
            {attendanceLoading ? (
              ...
              <div className="py-16 text-center flex flex-col items-center gap-4 bg-muted/10 rounded-[1.5rem] border-2 border-dashed">
                ...
                <p className="text-sm font-bold text-muted-foreground/60">'도면 수정' 버튼을 눌러 좌석을 배치해 주세요.</p>
              </div>
            ) : (
                    ...
                    return (
                      <Link key={seat.id} href={occupant ? `/dashboard/teacher/students/${occupant.id}` : "#"}>
                        <div className={cn(
                          ...
                          seat.status === 'break' ? "bg-blue-50 border-blue-400 text-blue-700" : "bg-white border-border text-muted-foreground"
                        )}>
                          <span className="text-[8px] sm:text-[10px] font-black opacity-40">{seat.seatNo}</span>
                          <span className="text-[9px] sm:text-[11px] font-black truncate px-1 w-full text-center">{occupant ? occupant.name : 'EMPTY'}</span>
      ...
      <Dialog open={isLayoutModalOpen} onOpenChange={setIsLayoutModalOpen}>
        <DialogContent className="max-w-[98vw] sm:max-w-[95vw] lg:max-w-6xl h-[95vh] sm:h-auto rounded-xl sm:rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl flex flex-col">
          <DialogHeader className="p-5 sm:p-8 bg-primary text-primary-foreground shrink-0 relative">
            ...
            <DialogDescription className="text-primary-foreground/70 font-bold text-xs sm:text-base">클릭하여 좌석을 배치하세요. 번호는 자동으로 매겨집니다.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="p-4 sm:p-8 bg-muted/30 border-t flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <Button onClick={saveLayout} disabled={isSaving || tempLayout.length === 0} className="flex-[2] sm:flex-none rounded-2xl font-black px-10 h-12 shadow-xl gap-2 active:scale-95 transition-all">
                {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} 저장 적용
              </Button>
            </div>
          </DialogFooter>
        </DialogContent>
